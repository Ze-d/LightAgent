...
Agent 执行过程中需要向客户端实时推送事件（工具调用开始/结束、最终答案）。客户端需要持续接收服务端推送，而不是轮询。
...
通过 `EventChannel`（内部基于 `asyncio.Queue`）实现事件发布，`/chat/stream` 返回 `EventSourceResponse`。

## 原因

1. **单向推送足够**：Agent 执行流不需要客户端反向发送消息，SSE 语义完全匹配
...
- **单向通信局限**：若未来需要客户端实时发送消息（如中断 Agent 执行），需引入 WebSocket 或其他机制
- **不支持跨域**：`EventSource` 默认不支持 CORS，需额外处理

### 技术债
- 尚未实现客户端断开连接时的 Agent 执行取消机制（目前 Agent 继续运行至结束）
- SSE 连接超时需前端配合处理，当前 `EventChannel` 关闭依赖服务端主动调用
