...
MyAgent 已支持 A2A（Agent-to-Agent）`1.0.0` 协议的核心 Server 与 Client 能力，默认使用 JSON-RPC transport，并保留旧 HTTP+JSON REST 端点用于兼容已有调用方。A2A 相关实现集中在 `app/a2a/`，`app/core/runner.py` 保持协议无关；A2A 层只负责协议模型、任务状态、HTTP 路由、事件流、Agent Card 和远端 Agent 调用适配。
...
| `app/a2a/client.py` | 同步 A2A Client，优先 JSON-RPC，兼容 HTTP+JSON |
...
| `POST` | `/a2a/v1/rpc` | A2A 1.0 JSON-RPC 入口，支持 `SendMessage`、`SendStreamingMessage`、`GetTask`、`ListTasks`、`CancelTask`、`SubscribeToTask`、`GetExtendedAgentCard` |
| `GET` | `/a2a/v1/extendedAgentCard` | 兼容旧 HTTP+JSON 的认证扩展 Agent Card |
| `POST` | `/a2a/v1/message:send` | 兼容旧 HTTP+JSON，发送消息并返回 Task |
| `POST` | `/a2a/v1/message:stream` | 兼容旧 HTTP+JSON，创建 Task 并通过 SSE 返回事件 |
...
```text
Task snapshot
-> statusUpdate TASK_STATE_CANCELED
```
...
`A2AClient` 是同步 Client。读取 Agent Card 后会优先选择 `JSONRPC` interface；远端只暴露旧 `HTTP+JSON` 时会自动回退。支持：
